...
const char *utf8_prev_char(const char *string_start, const char *string)
{
...
wint_t utf8_wide_char(const char *string)
{
	int char_size;
	wint_t result;
...
int utf8_strlen_screen(const char *string)
{
	...
	if (!string[1] || !string[2] || !string[3] || !string[4]) {
		...
		alloc_wstring =
			malloc((num_char + 1) * sizeof(alloc_wstring[0]));
		if (!alloc_wstring)
			return utf8_strlen(string);
...
int utf8_charcmp(const char *string1, const char *string2)
{
	...
	while ((i < length1) && (i < length2)) {
		diff = (int)((unsigned char)string1[i]) -
		       (int)((unsigned char)string2[i]);
		if (diff != 0)
			return (diff < 0) ? -1 : 1;
		i++;
	}
...
int utf8_charcasecmp_range(const char *string1, const char *string2, int range)
{
	wint_t wchar1, wchar2;
